...
export default function KBPage() {
  ...
  const [form, setForm] = useState({ question: "", answers: { en: "" }, tags: "", active: true });
  ...
  const save = async () => {
    try {
      await api.post("/admin/kb", { question: form.question, answers: form.answers, tags: form.tags.split(",").map(s=>s.trim()).filter(Boolean), active: true });
      toast.success("Saved"); setShowNew(false); setForm({question:"",answers:{en:""},tags:"",active:true}); load();
