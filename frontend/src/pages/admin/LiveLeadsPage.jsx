...
export default function LiveLeadsPage() {
  ...
  return (
    <div className="p-8">
      ...
      {/* Bulk actions bar — history tab only */}
        ...
        <div className="flex items-center gap-3 mb-4 p-3 bg-slate-50 rounded-2xl border border-slate-200">
          ...
          {selected.size > 0 && (
            <button
              ...
              onClick={bulkDelete}
              className="ml-auto flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold transition-colors"
            >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-4">
          {filtered.map((lead) => (
            <div key={lead.id} data-testid={`lead-${lead.id}`} className={`bg-white border rounded-3xl p-6 shadow-sm hover:border-orange-300 transition-colors ${selected.has(lead.id) ? "border-orange-400 bg-orange-50/30" : "border-slate-200"}`}>
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                {/* Select checkbox (history only) */}
                {tab === "history" && (
                ...
                <div className="space-y-2 min-w-0 flex-1">
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    ...
                    <div className="rounded-xl bg-slate-50 p-3">
                      ...
                      <div className="mt-1 text-sm text-slate-700">{lead.human_now ? "Immediate human" : "Scheduled"} · {lead.voice_requested ? "Voice" : "Text"}</div>
                    </div>
                  </div>
